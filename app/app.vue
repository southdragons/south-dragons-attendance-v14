...
const team = useTeam()
const { data, ready, storageError, isDemo, busy, admin, adminConfigured } = team
...
const players = computed(() => roster.value.filter(p => !onlyMine.value || data.value.myPlayerIds.includes(p.id)))
...
function status(eventId: string, playerId: string) { return data.value.attendance.find(a => a.eventId === eventId && a.playerId === playerId)?.status }
...
async function saveStatus(value: AttendanceStatus | null) {
  ...
  await perform(async () => { await team.saveAttendance(selection.event.id, selection.player.id, value); selected.value = null; notify(value ? '出欠を更新しました' : '未回答に戻しました') })
}
...
const adminTab = ref<'events' | 'players' | 'password'>('events')
...
async function saveEvent() {
  ...
  if (!draft.title.trim() || !draft.date || !draft.startTime || !draft.endTime || !draft.location.trim() || !draft.createdByName.trim()) { formError.value = '必須項目を入力してください。'; return }
...
<template>
  <div>
    ...
    <main class="page-shell">
      ...
      <template v-if="view === 'attendance'">
        ...
        <section class="schedule-section" aria-labelledby="schedule-title">
          ...
          <div class="table-bottom"><div class="legend"><span v-for="option in statusOptions" :key="option.value"><b :class="option.value">{{ option.symbol }}</b>{{ option.label }}</span><span><b class="unanswered">未</b>未回答</span></div><p>{{ history ? '過去の記録は閲覧のみです' : '出欠のマークをタップして変更' }}<span v-if="onlyMine"> · 集計はチーム全員</span></p></div>
        </section>
      ...
      <template v-else-if="admin">
        ...
        <div v-if="isDemo" class="admin-demo-note"><Info :size="18" />管理機能のお試し画面です。本番のパスワード認証はGAS接続後に有効になります。</div>
    ...
    <AppModal :busy="busy" :error="actionError" :open="registration" title="お子さまの名前を登録" @close="registration = false">
      <template v-if="duplicate"><div class="duplicate-note"><Users :size="30" /><h3>{{ duplicate.name }}さんは<br />すでに登録されています。</h3><p>{{ duplicate.active ? 'この選手を使用しますか？' : '退団扱いになっています。運営者に在籍への復帰を依頼してください。' }}</p></div><div class="modal-actions"><button class="btn btn-ghost" @click="duplicate = null">戻る</button><button v-if="duplicate.active" class="btn btn-primary" @click="usePlayer(duplicate)">この選手を使用する</button></div></template>
