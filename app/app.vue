...
import type { AttendanceStatus, Player, TeamEvent } from '~/types/team'
import { countsFor, createId, dateLabel, localDate, normalizeName, statusOptions, visiblePlayers } from '~/utils/team'
const team = useTeam()
const { data, ready, storageError, isDemo, isSupabase, busy, admin, adminConfigured, requests, pendingAttendance, myPendingRequests, pendingRequestCount } = team
...
function status(eventId: string, playerId: string) {
  const pending = team.pendingRequest(playerId)
...
async function saveStatus(value: AttendanceStatus | null) {
  ...
  const provisional = !!team.pendingRequest(selection.player.id) && !data.value.myPlayerIds.includes(selection.player.id)
...
function openRegistration() { name.value = ''; duplicate.value = null; formError.value = ''; actionError.value = ''; registration.value = true }
...
async function recoverPlayer(player: Player) {
  ...
  await perform(async () => { await team.requestRegistration(player.id); registration.value = false; notify('再登録を申請しました。承認待ちでも仮回答できます') })
}
function reviewRequest(id: string, playerName: string, decision: 'approved' | 'rejected') {
  confirmAction.value = { title: `${playerName}さんの申請を${decision === 'approved' ? '承認' : '却下'}しますか？`, message: decision === 'approved' ? '新しい端末に編集権限を移し、現在・未来の有効な予定への仮回答を正式に反映します。以前の端末は編集できなくなります。' : '現在の編集権限と正式な回答を保持します。仮回答は反映しません。', label: decision === 'approved' ? '承認する' : '却下する', action: async () => { await team.reviewRegistration(id, decision); notify('申請を処理しました') } }
}
...
const managedEvents = computed(() => [...data.value.events].sort((a,b) => a.date.localeCompare(b.date)))
...
function emptyEvent(): TeamEvent { return { id: createId(), date: localDate(), title: '', startTime: '08:00', endTime: '12:00', location: '', note: '', createdByName: '', active: true } }
...
async function saveEvent() {
  ...
  await perform(async () => { await team.saveEvent(record, creatingEvent.value); eventEditor.value = false; notify('予定を保存しました') })
}
  ...
  <div>
    ...
    <main class="page-shell">
      ...
      <template v-if="view === 'attendance'">
        ...
        <section v-if="requests.some(r => r.isMine && r.status === 'rejected' && !data.myPlayerIds.includes(r.playerId) && !team.pendingRequest(r.playerId))" class="alert alert-warning mb-4"><p>再登録申請が承認されなかった選手がいます。運営者に確認してください。</p></section>
        <section v-if="myPendingRequests.length" class="alert alert-info mb-4"><div><strong>再登録の承認待ち</strong><p v-for="request in myPendingRequests" :key="request.id">{{ request.playerName }}さん：仮回答を入力できます。集計には承認後に反映されます。</p></div></section>
        ...
        <section class="schedule-section" aria-labelledby="schedule-title">
          ...
          <div class="table-topline"><p><span class="live-dot" />{{ activePlayers.length }}名が在籍中<span class="subtle"> · {{ events.length }}件の予定</span></p><label v-if="myPlayers.length" class="mine-toggle"><input v-model="onlyMine" type="checkbox" class="checkbox checkbox-primary checkbox-sm" />自分の子どもだけ</label><span v-else class="scroll-hint">日程を横にスクロール <ArrowRight :size="14" /></span></div>
      ...
      <template v-else-if="admin">
        ...
        <button v-if="isSupabase" class="btn btn-outline mb-4" @click="adminTab = 'requests'">再登録申請 <span class="badge badge-primary">{{ pendingRequestCount }}件</span></button>
        <section v-if="adminTab === 'requests'" class="admin-panel card mb-4"><h2>再登録申請</h2><p v-if="!requests.length" class="py-4">申請はありません。</p><div v-for="request in requests" :key="request.id" class="management-row"><div><h3>{{ request.playerName }}</h3><p>{{ new Date(request.requestedAt).toLocaleString('ja-JP') }} · {{ request.status === 'pending' ? '承認待ち' : request.status === 'approved' ? '承認済み' : '却下済み' }}</p><p v-if="request.status === 'pending' && request.notificationStatus !== 'sent'" class="text-sm">LINE通知：{{ request.notificationStatus === 'unconfigured' ? '未設定' : request.notificationStatus === 'failed' ? '送信失敗' : '未送信' }}</p></div><div v-if="request.status === 'pending'" class="row-actions"><button class="btn btn-sm btn-primary" :disabled="busy" @click="reviewRequest(request.id, request.playerName, 'approved')">承認</button><button class="btn btn-sm btn-outline" :disabled="busy" @click="reviewRequest(request.id, request.playerName, 'rejected')">却下</button></div></div></section>
